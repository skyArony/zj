    ...
    <div id="survey"
      ...
      <el-row>
        ...
        <el-button type="primary"
                   size="small"
                   @click="getSurveyRecord()">查询</el-button>
      </el-row>
      <hr>
      <h3>
      ...
      <div style="height:700px; width:100%;">
        <canvas id="surveyChart"
                style="height:700px; width:100%"></canvas>
      </div>
    </div>
    <div id="tag"
      ...
      <div style="height:500px; width:500px; margin:40px auto">
        <canvas id="tagChart"
                style="height:500px; width:500px"></canvas>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getUserAllCourse() {
      let that = this
      this.MyAxios.get("/api/user/course").then(function(response) {
    ...
    getTagDetail() {
      ...
      this.MyAxios.get("/api/tag/detail/" + this.courseId).then(function(
        response
      ) {
  ...
  computed: {
    ...
    tagChartData: function() {
      ...
      for (let key in temp) {
        ...
        data[2].push('#'+('00000'+ (Math.random()*0x1000000<<0).toString(16)).substr(-6))
      }
