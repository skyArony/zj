    ...
    <el-col :span="17"
      ...
      <el-row>
        <worlduc-questionList :useDragHandle="true"
                              v-model="questions"
                              lockAxis="y">
          <worlduc-question v-for="(item, index) in questions"
                            :question="item"
                            :showHandle="true"
                            :key="item.id"
                            :index="index"
                            class="list-item"></worlduc-question>
...
export default {
  ...
  methods: {
    ...
    changeDynamicTags(option) {
      ...
      MyAxios.get("/tag", { params: { courseId: course_id } }).then(
        function(response) {
    ...
    pageInit() {
      ...
      if (this.pageType == "create") {
        ...
        let id = cookie.match(/teacher_id=(.*?);/)[1];
        let name = cookie.match(/name=(.*?);/)[1];
        this.name = name;
        // 获取所有的课程
        MyAxios.get("/course", {
          params: {
          ...
          .then(function(resposne) {
            let courses = resposne.data.data;
            let data = [];
            var index;
        ...
        // 获取问卷的 ID
        this.id = window.location.href.match(/.*\/(.*?)\/edit$/)[1];
        // 获取问卷的数据
        MyAxios.get("/survey", {
          params: {
            id: this.id
          ...
          .then(function(resposne) {
            that.ruleForm.title = resposne.data.data.title;
            that.ruleForm.desc = resposne.data.data.desc;
            that.$store.state.questions = JSON.parse(
              resposne.data.data.questions
            );
          });
      }
    },
