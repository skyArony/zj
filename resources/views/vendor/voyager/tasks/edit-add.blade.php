    ...
    <!-- 权限检查 -->
        @php
            ...
            if (!is_null($dataTypeContent->getKey()) && ($role == 3 || $role == 4)) {
                ...
                if ($creater_id != $dataTypeContent->creater_id) header("location:/admin/tasks/");
            }
        @endphp
    <!-- /权限检查 -->
@stop
...
@section('javascript')
    <script>
        ...
        function check(event) {
            ...
            if (!title || !desc || !detail || !regist_end_at || !request_end_at || !submit_end_at) {
                alert("除附件外,其他字段都为必填,且不能为空白符!")
                event.preventDefault();
            }
            // 时间控制
            var regist_end_at = $('.datepicker').eq(0).val();  // 报名截止时间
            var request_end_at = $('.datepicker').eq(1).val(); // 申请书提交截止时间
            var submit_end_at = $('.datepicker').eq(2).val(); // 成果提交截止时间
            if (regist_end_at < request_end_at && request_end_at < submit_end_at) ; // OK
            else {
                alert("截止时间设置错误,三个时间需满足从早到晚!")
                event.preventDefault();
            }
        }
    </script>
